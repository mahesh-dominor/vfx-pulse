...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("GET /api/shot-tasks error:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("POST /api/shot-tasks error:", error);
    return NextResponse.json({ error: "Failed to create shot task" }, { status: 500 });
  }
}
